...
export default class TransactionScreen extends React.Component {
    ...
    initiateBookIssue = async()=>{
      ...
      db.collection("Students").doc(this.state.scannedStudentId).update({
        'BooksInPossession': firebase.firestore.FieldValue.increment(1)
      })
    }
    ...
    checkBookEligibility = async () => {
      const bookRef = await db
        .collection("Books")
        .where("ID", "==", this.state.scannedBookId)
        .get();
      var transactionType = "";
    ...
    checkStudentEligibilityForBookIssue = async () => {
      const studentRef = await db
        .collection("Students")
        .where("ID", "==", this.state.scannedStudentId)
        .get();
      ...
      if (studentRef.docs.length == 0) {
        ...
        studentRef.docs.map(doc => {
          ...
          if (student.BooksInPossession < 2) {
            isStudentEligible = true;
          }
          else {
            isStudentEligible = false;
            Alert.alert("The student has already checked out 2 books");
            this.setState({
              scannedStudentId: "",
              scannedBookId: ""
            });
    ...
    checkStudentEligibilityForReturn = async () => {
      ...
      transactionRef.docs.map(doc => {
        ...
        if (lastBookTransaction.StudentID === this.state.scannedStudentId) {
          ...
          isStudentEligible = false;
          Alert.alert("The book wasn't checked out by this student");
          this.setState({
            scannedStudentId: "",
            scannedBookId: ""
          });
    ...
    handleTransaction = async () => {
      ...
      if (!transactionType) {
        ...
        if (isStudentEligible) {
          ...
          Alert.alert("Book checked out by the student");
        }
      }
      else {
